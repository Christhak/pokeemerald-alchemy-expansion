    ...
    if (!gDisableTextPrinters)
    {
        for (i = 0; i < WINDOWS_MAX; ++i)
        {
            if (sTextPrinters[i].active)
            {
                ...
                CopyWindowToVram(sTextPrinters[i].printerTemplate.windowId, COPYWIN_GFX); // Move this line up
                switch (renderCmd)
                {
                case RENDER_PRINT:
                    // Removed the old CopyWindowToVram call here
                case RENDER_UPDATE:
...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        } while (--repeats > 0 && *textPrinter->printerTemplate.currentChar != EOS);
